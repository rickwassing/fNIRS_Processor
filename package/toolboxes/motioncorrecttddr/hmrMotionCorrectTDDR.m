% function dodTDDR = hmrMotionCorrectTDDR(dod)
%
...
%
% OUTPUTS:
% dod:     dod after correction via TDDR
%
% LOG:
% Script by Frank Fishburn (fishburnf@upmc.edu) 10/03/2018
%

function [dod] = hmrMotionCorrectTDDR(dod)

sample_rate = 1/mean(diff(dod.time));

for ii = 1:length(dod.measurementList)
    ...
    if Fc<1
        ...
        signal_low = filtfilt(fb, fa, dod.dataTimeSeries(:, ii));
    else
        signal_low = dod.dataTimeSeries(:, ii);
    end
    signal_high = dod.dataTimeSeries(:, ii) - signal_low;
    ...
    dod.dataTimeSeries(:,ii) = signal_low_corrected + signal_high;
