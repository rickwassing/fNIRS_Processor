% SYNTAX:
% [data_avg, data_std, nTrials, data_sum2, yTrials] = fni_blockavg( data, stim, trange )
%
% UI NAME:
% Block_Average
%
...
%
% USAGE OPTIONS:
% Block_Average_on_Concentration_Data: [dcAvg, dcAvgStd, nTrials, dcSum2] = fni_blockavg( dc, stim, trange )
% Block_Average_on_Delta_OD_Data: [dodAvg, dodAvgStd, nTrials, dodSum2] = fni_blockavg( dod, stim, trange )
%
% PARAMETERS:
% trange: [-2.0, 20.0]
%
...
%
function [data_avg, data_std, nTrials, data_sum2, yTrials] = fni_blockavg( data, stim, trange )
...
for kk=1:length(data)
    ...
    for iC = 1:size(s,2)
        ...
        % ---------------------------------------------------------------
        % EDIT: Insert this code to remove baseline prior to calculating
        % the standard deviation
        yblk = yblk - repmat(mean(yblk(1:-nPre, :, :, :), 1), length(tHRF), 1, 1, 1);
        % ---------------------------------------------------------------
