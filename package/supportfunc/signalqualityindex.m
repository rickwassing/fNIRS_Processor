% SIGNALQUALITYINDEX
% Returns a scalar value representing the signal quality score.
%
% Use as SQIscore = signalqualityindex(OD1, OD2, oxy, dxy, Fs)
% where
...
function SQIscore = signalqualityindex(OD1, OD2, oxy, dxy, Fs)
...
[OD1_filt, ~, ~] = ft_preproc_bandpassfilter((OD1), Fs, [0.4, min([Fs/2-0.001, 3])], [], 'firws', 'onepass-zerophase',[],[],[],[],[],[]);
[OD2_filt, ~, ~] = ft_preproc_bandpassfilter((OD2), Fs, [0.4, min([Fs/2-0.001, 3])], [], 'firws', 'onepass-zerophase',[],[],[],[],[],[]);
[oxy_filt, ~, ~] = ft_preproc_bandpassfilter((oxy), Fs, [0.4, min([Fs/2-0.001, 3])], [], 'firws', 'onepass-zerophase',[],[],[],[],[],[]);
[dxy_filt, ~, ~] = ft_preproc_bandpassfilter((dxy), Fs, [0.4, min([Fs/2-0.001, 3])], [], 'firws', 'onepass-zerophase',[],[],[],[],[],[]);
